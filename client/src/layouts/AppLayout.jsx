...
function Navigation({ onNavigate }) {
  return <nav className="mt-6 grid gap-2">
    {nav.map(([to, label]) => <NavLink key={to} to={to} onClick={onNavigate} className={({ isActive }) => `rounded-lg px-3 py-2 text-sm ${isActive ? 'bg-accent text-text-inverted' : 'text-text-secondary hover:bg-elevated'}`}>{label}</NavLink>)}
  </nav>;
}
...
export function AppLayout() {
  ...
  return <div className="min-h-screen bg-app text-text-primary md:flex">
    ...
    <aside className="hidden border-r border-border-subtle bg-panel/80 p-4 md:block md:min-h-screen md:w-64 md:shrink-0">
      <Brand />
      <Navigation />
