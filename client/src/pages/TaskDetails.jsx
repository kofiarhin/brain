...
=======
  ...
  if (taskQuery.isError) return <div className="space-y-4">
    ...
    <div className="rounded-lg border border-danger/40 bg-danger-soft/45 p-4 text-danger">Task could not be loaded.</div>
  </div>;

  return <div className="min-h-[calc(100vh-5rem)] overflow-hidden rounded-2xl border border-border-subtle bg-app">
    <div className="border-b border-border-subtle p-4">
      <Link to="/tasks" className="inline-flex min-h-11 items-center rounded-lg border border-border px-3 text-sm text-text-secondary hover:bg-panel focus:outline-none focus:ring-2 focus:ring-accent">Back to tasks</Link>
    </div>
    <TaskDetailPanel task={task} projects={projectsQuery.data || []} actions={actions} onEnterExecution={setExecutionTask} onBack={() => navigate('/tasks')} compact />
    {executionTask ? <div className="fixed inset-0 z-50 bg-app p-6">
      <div className="mx-auto max-w-2xl">
        <button type="button" onClick={() => setExecutionTask(null)} className="min-h-11 rounded-lg border border-border px-4 text-sm text-text-secondary hover:bg-panel">Exit execution mode</button>
        <h1 className="mt-8 break-words text-3xl font-semibold">{task.title}</h1>
